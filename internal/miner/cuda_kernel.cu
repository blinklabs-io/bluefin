...
extern "C" int bluefin_cuda_search(bluefin_cuda_backend*b,const uint8_t*state,int len,uint32_t lz,uint32_t diff,uint32_t round,const uint8_t*base,uint8_t*nonce,uint8_t*hash){
    if(!b||len<=0||len>MAX_STATE_LEN){snprintf(last_error,sizeof(last_error),"invalid CUDA search arguments");return -1;}cudaError_t e=cudaMemcpy(b->state,state,len,cudaMemcpyHostToDevice);if(e==cudaSuccess)e=cudaMemcpy(b->base_nonce,base,16,cudaMemcpyHostToDevice);if(e==cudaSuccess)e=cudaMemset(b->result,0,13*sizeof(uint32_t));if(e!=cudaSuccess){set_error(e,"CUDA upload");return -1;}int blocks=(b->batch_size+THREADS_PER_BLOCK-1)/THREADS_PER_BLOCK;tuna_search<<<blocks,THREADS_PER_BLOCK>>>(b->state,len,b->base_nonce,round,lz,diff,b->result);e=cudaGetLastError();if(e==cudaSuccess)e=cudaDeviceSynchronize();if(e!=cudaSuccess){set_error(e,"CUDA kernel");return -1;}uint32_t r[13];e=cudaMemcpy(r,b->result,sizeof(r),cudaMemcpyDeviceToHost);if(e!=cudaSuccess){set_error(e,"CUDA download");return -1;}if(!r[0])return 0;for(int i=0;i<4;i++)for(int j=0;j<4;j++)nonce[i*4+j]=(uint8_t)(r[1+i]>>(24-8*j));for(int i=0;i<8;i++)for(int j=0;j<4;j++)hash[i*4+j]=(uint8_t)(r[5+i]>>(24-8*j));return 1;
}
extern "C" void bluefin_cuda_destroy(bluefin_cuda_backend*b){if(!b)return;cudaFree(b->state);cudaFree(b->base_nonce);cudaFree(b->result);delete b;}
